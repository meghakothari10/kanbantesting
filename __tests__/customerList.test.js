...
describe('CustomerList component', () => {
  ...
  const averageSpending = customers.reduce((acc, curr) => acc + curr.spending, 0) / customers.length;
  ...
  test('renders customer list', async () => {
    ...
    await waitFor(() => expect(getByText('Top 5 Customers Who Spent More Than Average Spending')).toBeInTheDocument());
  ...
  test('displays correct customer names', async () => {
    const { getAllByTestId } = render(<CustomerList customers={topCustomers} />);
    const customerNames = getAllByTestId('customer-name');
    expect(customerNames.length).toBe(5);
    topCustomers.forEach((customer, index) => {
      expect(customerNames[index]).toHaveTextContent(customer.name);
    });
  });

  test('displays correct customer spending', async () => {
    const { getAllByTestId } = render(<CustomerList customers={topCustomers} />);
    const customerSpending = getAllByTestId('customer-spending');
    expect(customerSpending.length).toBe(5);
    topCustomers.forEach((customer, index) => {
      expect(customerSpending[index]).toHaveTextContent(`Spending: ${customer.spending}`);
    });
  });
